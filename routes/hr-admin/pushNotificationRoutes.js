...
router.post("/getNotificationList", async (req, res) => {
  try {
    ...
    const notifications = await PushNotification.find({ EMPNO }).sort({
      sentAt: -1,
    });
...
router.post("/getNotificationinfo", async (req, res) => {
  try {
    console.log("=======req.body", req.body);
    const notifications = await LeaveDetail.findOne({
      LVAPNO: req.body.LVAPNO,
    });
    return res.status(200).json({
      Status: "Success",
      Message: "Notification Viewd successfully",
      Data: notifications,
      Code: 200,
    });
  } catch (error) {
