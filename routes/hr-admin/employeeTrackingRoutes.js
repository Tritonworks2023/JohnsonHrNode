...
router.post("/list-tracking-notification", async (req, res) => {
  try {
    ...
    if (!EMPNO) {
      return res
        .status(400)
        .json({ Status: "Failed", Message: "EMPNO is required" });
    }
    ...
    if (sortedAttendance && sortedAttendance.CHECKINSTATUS) {
      const notification = await TrackingNotification.findOne({
        EMPNO,
        STATUS: "Not View",
        TYPE: "USER",
      });
...
router.post("/update-tracking-notification", async (req, res) => {
  try {
    ...
    if (!EMPNO || !notificationId) {
      return res.status(400).json({
        Status: "Failed",
        Message: "EMPNO, notificationId are required fields",
      });
    }
    const updatedNotifications = await TrackingNotification.updateMany(
      { EMPNO },
      { STATUS: "View" },
    );
    return res.status(200).json({
...
router.post("/list-employee-tracking", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      Status: "Success",
      Message: "Employee tracking records fetched successfully",
...
router.post("/create-Live-tracking", async (req, res) => {
  try {
    ...
    const employeeData = await EmployeeMaster.findOne({ ECODE: EMPNO }).select(
      "LOCCODE",
    );
    if (employeeData.LOCCODE == "NONFLD") {
      const branchRecord = await BranchMaster.findOne({ BRCODE });
